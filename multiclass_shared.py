...
class Simulation:
    def __init__(self,pev_num, lam, s, r, soc_rs, soc_i_mu=SOC_I_MU,soc_i_sigma=SOC_I_SIGMA,p1_max=P1_MAX, p2_max = P2_MAX, e_max=E_MAX,e1_c=E1_C, e2_c = E2_C):
        ...
        for soc_r1 in self.soc_rs:
            ...
            self.env.process(self.run_charging_station1(stop_event=stop_event))
        ...
        for soc_r2 in self.soc_rs:
            ...
            self.soc_r2 = soc_r2
            stop_event = Event(self.env)
            self.env.process(self.run_charging_station2(stop_event=stop_event))
            self.env.run(stop_event)
            self.temp_pevs = pd.DataFrame(self.temp_pevs)
            self.temp_pevs.set_index("pev", inplace = True)
    ...
    def run_charging_station1(self, stop_event):
        charging_station = ChargingStation(self.env, self.s1, self.r)
        admission = True
        i1 = 0
        while True:
    ...
    def run_charging_station2(self, stop_event):
        charging_station = ChargingStation(self.env, self.s2, self.r)
        admission = True
        i2 = 0
        while True:
            #TODO try a different way of doing this timeout (to mediate the discrepancies with the results from the paper)
