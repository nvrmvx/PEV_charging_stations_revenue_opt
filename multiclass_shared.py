...
class Simulation:
    def __init__(self,pev_num, lam, s, r, soc_rs, batt_deg, reward, c_w, t_ch_coefficient, soc_i_mu=SOC_I_MU,soc_i_sigma=SOC_I_SIGMA,p1_max=P1_MAX, p2_max = P2_MAX, e_max=E_MAX,e1_c=E1_C, e2_c = E2_C):
        self.lam = lam
        ...
        for lam1 in self.lam:
            ...
            self.env.process(self.run_charging_station1())
        ...
        for lam2 in self.lam:
            ...
            self.lam2 = lam2
            self.stop_event = Event(self.env)
            self.env.process(self.run_charging_station2())
            self.env.run(self.stop_event)
            self.temp_pevs = pd.DataFrame(self.temp_pevs)
            self.temp_pevs.set_index("pev", inplace = True)
    ...
    def run_charging_station1(self):
        ...
        while True:
            ...
            yield self.env.timeout(random.expovariate(self.lam1/60))
            i1 += 1
            if charging_station.admission:
                ...
                self.env.process(pev1.go_to_charging_station(self.env,charging_station))
            if i1 >= self.pev1_num:
    ...
    def run_charging_station2(self):
        ...
        while True:
            #TODO try a different way of doing this timeout (to mediate the discrepancies with the results from the paper)
            # wait time until next PEV has to be introduced to the simulation
            yield self.env.timeout(random.expovariate(self.lam2/60))
            i2 += 1
            if charging_station.admission:
                # create a new PEV in the simulation and send it to the charging station
                pev2 = Pev(self.soc_r2, i2, self.e2_c, self.p2_max, self)
                self.env.process(pev2.go_to_charging_station(self.env,charging_station))
            if i2 >= self.pev2_num:
                charging_station.admission = False
    ...
    def get_system_revenue1(self):
        ...
        for lam1 in self.lam:
            ...
            temp1[lam1] = theta1*self.lam1*(1-p_k[lam1])*(reward(lam1*theta2)-self.c_w*mean_t_w[lam1]/60.0-mean_c_batt*mean_t_ch[lam1]/60.0)
        plt.subplot(3,2,5)
        plt.plot(list(temp1.keys()), list(temp1.values()))
        plt.show()
        return temp1
